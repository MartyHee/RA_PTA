"""
real_raw_5000 数据读取与读取检查

读取 11 张 raw CSV 表，输出表级摘要和读取报告。

用法:
    D:/CodeData/software/Anaconda/Anaconda3/envs/ra/python.exe src/data/load_real_raw_5000.py --config configs/common/real_raw_5000.yaml
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="real_raw_5000 数据读取与读取检查")
    parser.add_argument(
        "--config",
        type=str,
        default=str(PROJECT_ROOT / "configs" / "common" / "real_raw_5000.yaml"),
        help="real_raw_5000 配置文件路径",
    )
    ...
    print(f"[load_real_raw_5000] 加载配置文件: {config_path}")
    ...
    print(f"[load_real_raw_5000] 数据根目录: {root_dir}")

    if not root_dir.exists():
        print(f"[load_real_raw_5000] 错误: 数据目录不存在: {root_dir}")
        sys.exit(1)

    # 输出目录
    outputs_dir = PROJECT_ROOT / "outputs"
    data_check_dir = outputs_dir / "data_check" / "real_raw_5000"
    ...
    print(f"\n[load_real_raw_5000] 开始读取 {len(tables_cfg)} 张 raw 表...\n")
    ...
    if "video_detail" in loaded_dfs:
        ...
        if "video_id" in vd.columns:
            ...
            print(f"\n[load_real_raw_5000] raw_video_detail.video_id unique: {n_unique_video_id}")
            if n_unique_video_id != expected_unique:
                print(f"[load_real_raw_5000] 警告: video_id unique 数 ({n_unique_video_id}) "
                      f"不等于期望值 ({expected_unique})")

    # =========================================================================
    # 4. 跨表关联检查
    # =========================================================================
    print("\n[load_real_raw_5000] 跨表关联检查...")
    ...
    # 5. 输出表级摘要 CSV
    # =========================================================================
    summary_csv_path = data_check_dir / "real_raw_5000_table_summary.csv"
    summary_rows = []
    ...
    print(f"\n[load_real_raw_5000] 表级摘要已保存: {summary_csv_path}")
    ...
    report: dict[str, Any] = {
        ...
        "next_step": "完成读取检查后，下一步可基于 real_raw_5000 构建 no_interaction_leakage 版 tabular 数据集。",
    }

    report_json_path = data_check_dir / "real_raw_5000_load_report.json"
    with open(report_json_path, "w", encoding="utf-8") as f:
        json.dump(report, f, ensure_ascii=False, indent=2)
    print(f"[load_real_raw_5000] 读取报告已保存: {report_json_path}")
    ...
    if critical_ok:
        print("[load_real_raw_5000] 所有关键表非空检查通过。")
    else:
        print("[load_real_raw_5000] 部分关键表为空或未读取，请检查。")

    # 退出码
    if len(failed_tables) > 0 or len(missing_files) > 0:
        print(f"\n[load_real_raw_5000] 部分表读取失败，请检查。")
        sys.exit(1)
    else:
        print(f"\n[load_real_raw_5000] 全部读取完成。")
