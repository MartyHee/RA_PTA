"""
统一 Tabular 数据集构建入口 — 支持通过 --dataset 读取任意已注册数据集。

基于 real_raw 系列 11 张 raw 表构建 DNN / Wide & Deep 可用的 tabular 输入，
使用 train / val / test 三路切分。

核心设计:
  1. no_interaction_leakage 是强制规则内置。
     标签源字段 digg_count/comment_count/share_count/collect_count
     不可通过配置关闭排除。
  2. 标签用 interaction_score 分位数构造。
  3. 潜在泄漏字段不进入模型特征。
  4. 泄漏检查在输出前强制执行，发现问题报错中止。
  5. 质量过滤 mode=full（全部样本）或 high_confidence（高质量子集）。

用法:
    D:/CodeData/software/Anaconda/Anaconda3/envs/ra/python.exe src/data/build_tabular.py --dataset real_raw_5000
    D:/CodeData/software/Anaconda/Anaconda3/envs/ra/python.exe src/data/build_tabular.py --config configs/common/feature_tabular_real_raw_5000.yaml
"""
...
# 强制泄漏控制 — 硬编码不可配置
# =============================================================================
# 这些字段是 label source 字段，只允许用于构造 interaction_score 和 label，
# 不得出现在任何模型输入中。此设定不可通过配置关闭。
...
def resolve_config(dataset_name: str | None, config_path: str | None) -> tuple[dict[str, Any], str, str]:
    """解析配置来源：--dataset 或 --config（互斥）。

    返回 (config_dict, dataset_name, config_source_desc)。
    """
    if dataset_name and config_path:
        print("[build_tabular] 错误: --dataset 和 --config 不能同时指定。")
        sys.exit(1)
    if not dataset_name and not config_path:
        print("[build_tabular] 错误: 必须指定 --dataset 或 --config。")
        print("  示例: python src/data/build_tabular.py --dataset real_raw_5000")
        print("  示例: python src/data/build_tabular.py --config configs/common/feature_tabular_real_raw_5000.yaml")
        sys.exit(1)

    if dataset_name:
        datasets_yaml = PROJECT_ROOT / "configs" / "datasets.yaml"
        if not datasets_yaml.exists():
            print(f"[build_tabular] 错误: datasets.yaml 不存在: {datasets_yaml}")
            sys.exit(1)
        with open(datasets_yaml, "r", encoding="utf-8") as f:
            registry = yaml.safe_load(f) or {}
        datasets = registry.get("datasets", {})
        if dataset_name not in datasets:
            print(f"[build_tabular] 错误: 数据集 '{dataset_name}' 未在 configs/datasets.yaml 中注册。")
            print(f"  已注册: {list(datasets.keys())}")
            sys.exit(1)

        tabular_config_rel = datasets[dataset_name].get("tabular_config")
        if not tabular_config_rel:
            print(f"[build_tabular] 错误: 数据集 '{dataset_name}' 未配置 tabular_config。")
            sys.exit(1)

        tabular_config_path = PROJECT_ROOT / tabular_config_rel
        if not tabular_config_path.exists():
            print(f"[build_tabular] 错误: tabular_config 文件不存在: {tabular_config_path}")
            sys.exit(1)

        cfg = load_config(str(tabular_config_path))

        # 合并 dataset-level tabular_params（覆盖共享配置中的同名字段）
        tabular_params = datasets[dataset_name].get("tabular_params")
        if tabular_params is None:
            print(f"[build_tabular] 错误: 数据集 '{dataset_name}' 未配置 tabular_params。")
            print(f"  请确保在 configs/datasets.yaml 中为 {dataset_name} 定义了 tabular_params 块。")
            sys.exit(1)
        merge_tabular_params(cfg, tabular_params, dataset_name)

        resolved_dataset = dataset_name
        source_desc = f"--dataset {dataset_name} -> {tabular_config_rel}"
    else:
        cfg_path = Path(config_path)  # type: ignore[arg-type]
        if not cfg_path.exists():
            print(f"[build_tabular] 错误: 配置文件不存在: {cfg_path}")
            sys.exit(1)
        cfg = load_config(str(cfg_path))
        resolved_dataset = cfg.get("dataset_name", cfg_path.stem)
        source_desc = f"--config {config_path}"

    return cfg, resolved_dataset, source_desc


def merge_tabular_params(
    cfg: dict[str, Any],
    tabular_params: dict[str, Any],
    dataset_name: str,
) -> None:
    """将 dataset-level tabular_params 合并到共享配置 cfg。

    tabular_params 为扁平结构（data_root, file_suffix, features_dir 等），
    cfg 为嵌套结构（output_paths, quality_filter 等）。
    本函数将扁平键映射到对应嵌套位置并覆盖。
    """
    # data_root → cfg["data_root"]
    if "data_root" in tabular_params:
        cfg["data_root"] = tabular_params["data_root"]

    # file_suffix → 格式化 tables 中的文件名模板
    file_suffix = tabular_params.get("file_suffix", "")
    if file_suffix and "tables" in cfg:
        for key in list(cfg["tables"].keys()):
            cfg["tables"][key] = cfg["tables"][key].replace("{file_suffix}", file_suffix)

    # features_dir / check_dir → cfg["output_paths"]
    if "output_paths" not in cfg:
        cfg["output_paths"] = {}
    if "features_dir" in tabular_params:
        cfg["output_paths"]["features_dir"] = tabular_params["features_dir"]
    if "check_dir" in tabular_params:
        cfg["output_paths"]["check_dir"] = tabular_params["check_dir"]

    # high_confidence_video_ids → cfg["quality_filter"]
    if "high_confidence_video_ids" in tabular_params and "quality_filter" in cfg:
        cfg["quality_filter"]["high_confidence_video_ids"] = tabular_params["high_confidence_video_ids"]

    # expected_unique_video_ids 可选字段
    if "expected_unique_video_ids" in tabular_params:
        cfg["expected_unique_video_ids"] = tabular_params["expected_unique_video_ids"]
...
def validate_leakage(
    ...
    if leaked_in_csv:
        errors.append(f"label source 字段仍存在于最终 CSV 中: {leaked_in_csv}")
    ...
    if leaked_in_features:
        errors.append(f"label source 字段仍存在于 feature_info 特征列表中: {leaked_in_features}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="统一 Tabular 数据集构建入口 — no_interaction_leakage 强制内置")
    parser.add_argument(
        "--dataset",
        type=str,
        default=None,
        help="数据集名称（从 configs/datasets.yaml 读取配置）",
    )
    parser.add_argument(
        "--config",
        type=str,
        default=None,
        help="配置文件路径（与 --dataset 互斥）",
    )
    args = parser.parse_args()

    print("=" * 60)
    print("Tabular 数据集构建 (no_interaction_leakage 强制内置)")
    print("=" * 60)

    # =========================================================================
    # 1. 加载配置
    # =========================================================================
    cfg, dataset_name, config_source = resolve_config(args.dataset, args.config)
    print(f"\n[1/11] 加载配置...")
    print(f"  配置来源: {config_source}")
    print(f"  数据集: {dataset_name}")

    run_id = cfg.get("run_id", dataset_name)
    ...
    # 泄漏控制参数：合并硬编码字段与配置字段
    # 硬编码字段不可通过配置移除
    config_excluded = leak_cfg.get("excluded_label_source_features", [])
    merged_excluded = list(dict.fromkeys(HARDCODED_EXCLUDED_LABEL_SOURCE + config_excluded))
    if set(HARDCODED_EXCLUDED_LABEL_SOURCE) - set(merged_excluded):
        print("[build_tabular] 错误: 硬编码泄漏字段被配置移除，此情况不应发生。")
        sys.exit(1)
    excluded_label_source = merged_excluded
    ...
    features_dir = PROJECT_ROOT / output_paths_cfg.get("features_dir", f"data/features/{dataset_name}")
    check_dir = PROJECT_ROOT / output_paths_cfg.get("check_dir", f"outputs/data_check/{dataset_name}")
    ...
    print(f"  泄漏控制(强制): {HARDCODED_EXCLUDED_LABEL_SOURCE}")
    ...
    # 排除低质量列（注意：always_keep 不包含 comment_digg_count，属潜在泄漏）
    ...
    feature_info = {
        ...
        "excluded_label_source_features": excluded_label_source,
        "excluded_potential_leakage_cols": excluded_potential_leakage,
        "hardcoded_excluded_label_source": HARDCODED_EXCLUDED_LABEL_SOURCE,
        ...
        "warnings": warnings,
        "known_limitations": [
            f"{dataset_name} 来自公开网页端，不代表平台内部完整数据。",
        ...
        "notes": [
            f"本数据集基于 {dataset_name} 真实网页端 raw 数据构建 (quality_filter_mode={qf_mode})。",
    ...
    report_path = check_dir / f"{dataset_name}_tabular_dataset_report.json"
    with open(report_path, "w", encoding="utf-8") as f:
        json.dump(report, f, ensure_ascii=False, indent=2)
    print(f"  tabular_dataset_report.json: {report_path}")

    # 11d. preview CSV
    preview_path = check_dir / f"{dataset_name}_tabular_dataset_preview.csv"
    ...
    leakage_report_path = check_dir / f"{dataset_name}_leakage_check_report.json"
