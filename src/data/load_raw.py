"""
统一数据读取入口 — 支持通过 --dataset 读取任意已注册数据集。

用法:
    python src/data/load_raw.py --dataset real_raw_5000
    python src/data/load_raw.py --config configs/common/real_raw_5000.yaml
"""
...
def resolve_config(dataset_name: str | None, config_path: str | None) -> tuple[dict[str, Any], str, str]:
    ...
    if dataset_name and config_path:
        print("[load_raw] 错误: --dataset 和 --config 不能同时指定。")
        sys.exit(1)
    if not dataset_name and not config_path:
        print("[load_raw] 错误: 必须指定 --dataset 或 --config。")
        print("  示例: python src/data/load_raw.py --dataset real_raw_5000")
        print("  示例: python src/data/load_raw.py --config configs/common/real_raw_5000.yaml")
        sys.exit(1)

    if dataset_name:
        # 从 datasets.yaml 读取配置路径
        datasets_yaml = PROJECT_ROOT / "configs" / "datasets.yaml"
        if not datasets_yaml.exists():
            print(f"[load_raw] 错误: datasets.yaml 不存在: {datasets_yaml}")
            sys.exit(1)
        with open(datasets_yaml, "r", encoding="utf-8") as f:
            registry = yaml.safe_load(f) or {}
        datasets = registry.get("datasets", {})
        if dataset_name not in datasets:
            print(f"[load_raw] 错误: 数据集 '{dataset_name}' 未在 configs/datasets.yaml 中注册。")
            print(f"  已注册: {list(datasets.keys())}")
            sys.exit(1)

        raw_config_rel = datasets[dataset_name].get("raw_config")
        if not raw_config_rel:
            print(f"[load_raw] 错误: 数据集 '{dataset_name}' 未配置 raw_config。")
            sys.exit(1)

        raw_config_path = PROJECT_ROOT / raw_config_rel
        if not raw_config_path.exists():
            print(f"[load_raw] 错误: raw_config 文件不存在: {raw_config_path}")
            sys.exit(1)

        cfg = load_config(str(raw_config_path))
        resolved_dataset = dataset_name
        source_desc = f"--dataset {dataset_name} -> {raw_config_rel}"
    else:
        # 直接使用 --config
        cfg_path = Path(config_path)  # type: ignore[arg-type]
        if not cfg_path.exists():
            print(f"[load_raw] 错误: 配置文件不存在: {cfg_path}")
            sys.exit(1)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="统一数据读取入口 — 支持通过 --dataset 或 --config 指定数据集")
    parser.add_argument(
        "--dataset",
        type=str,
        default=None,
    ...
    print(f"[load_raw] 配置来源: {config_source}")
    print(f"[load_raw] 数据集: {dataset_name}")

    run_id = cfg.get("run_id", dataset_name)
    root_rel = cfg["root"]
    delivery_doc = cfg.get("delivery_doc", "")
    expected_unique = cfg.get("expected_unique_video_ids", 5000)
    ...
    print(f"[load_raw] 数据根目录: {root_dir}")

    if not root_dir.exists():
        print(f"[load_raw] 错误: 数据目录不存在: {root_dir}")
        sys.exit(1)

    # 输出目录（数据集专属子目录）
    outputs_dir = PROJECT_ROOT / "outputs"
    data_check_dir = outputs_dir / "data_check" / dataset_name
    ...
    print(f"\n[load_raw] 开始读取 {len(tables_cfg)} 张 raw 表...\n")
    ...
    for table_key in table_keys:
        ...
        try:
            ...
            # 允许空表但 CSV 无列可解析时，视为空表而非失败
            if is_allowed_empty and "No columns to parse from file" in str(e):
                empty_df = pd.DataFrame()
                print(f"OK | 0 行 x 0 列（允许空表，CSV 无列）")
                info = build_table_info(
                    table_key=table_key,
                    file_path=file_path,
                    read_success=True,
                    encoding_used="N/A (empty)",
                    df=empty_df,
                    error_message=None,
                    allowed_empty=is_allowed_empty,
                )
                loaded_dfs[table_key] = empty_df
                loaded_tables.append(table_key)
                empty_tables.append(table_key)
            else:
                print(f"失败: {e}")
    ...
    if "video_detail" in loaded_dfs:
        ...
        if "video_id" in vd.columns:
            ...
            print(f"\n[load_raw] raw_video_detail.video_id unique: {n_unique_video_id}")
            if n_unique_video_id != expected_unique:
                print(f"[load_raw] 警告: video_id unique 数 ({n_unique_video_id}) "
                      f"不等于期望值 ({expected_unique})")
    ...
    # 5. 输出表级摘要 CSV
    # =========================================================================
    summary_csv_path = data_check_dir / f"{dataset_name}_table_summary.csv"
    summary_rows = []
    ...
    print(f"\n[load_raw] 表级摘要已保存: {summary_csv_path}")
    ...
    known_limitations = [
        ...
        "none-match 样本占 30.1%，部分互动和媒体字段覆盖率偏低。",
        "当前不包含真实曝光、点击、完播、转化标签。",
        "当前来自公开网页端数据，不代表平台内部完整数据。",
        "评论和相关推荐不是全量触发（comment 37.5%，related 45.3%）。",
    ]
    ...
    report: dict[str, Any] = {
        ...
        "delivery_doc": str(root_dir / delivery_doc) if delivery_doc else "",
        ...
        "next_step": f"完成读取检查后，下一步可基于 {dataset_name} 构建 tabular 数据集。",
    }

    report_json_path = data_check_dir / f"{dataset_name}_load_report.json"
    with open(report_json_path, "w", encoding="utf-8") as f:
        json.dump(report, f, ensure_ascii=False, indent=2)
    print(f"[load_raw] 读取报告已保存: {report_json_path}")
    ...
    if critical_ok:
        print("[load_raw] 所有关键表非空检查通过。")
    else:
        print("[load_raw] 部分关键表为空或未读取，请检查。")

    # 退出码
    if len(failed_tables) > 0 or len(missing_files) > 0:
        print(f"\n[load_raw] 部分表读取失败，请检查。")
        sys.exit(1)
    else:
        print(f"\n[load_raw] 全部读取完成。")
