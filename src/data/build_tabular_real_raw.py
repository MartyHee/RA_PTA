"""
real_raw_1000 Tabular 数据集构建脚本

基于 real_raw_1000 的 11 张 raw 表构建 DNN / Wide & Deep 可用的 tabular 输入，
使用 train / val / test 三路切分。

用法:
    D:/CodeData/software/Anaconda/Anaconda3/envs/ra/python.exe src/data/build_tabular_real_raw.py
        --config configs/common/feature_tabular_real_raw_1000.yaml

设计原则:
    - 以 raw_video_detail 为主表，video_id 粒度。
    - 字段排除基于数据质量动态计算，不硬编码。
    - raw_author 去重后再 LEFT JOIN（同一 author 多视频产生重复行）。
    - raw_video_tag / raw_chapter 为空表，自动跳过聚合。
    - 标签用 interaction_score = digg+comment+share+collect 分位数。
    - train/val/test = 70/15/15，按 video_id 切分，支持分层。
    - test 仅用于最终泛化评估，不参与调参。
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="real_raw_1000 Tabular 数据集构建")
    parser.add_argument(
        "--config",
        type=str,
        default=str(PROJECT_ROOT / "configs" / "common" / "feature_tabular_real_raw_1000.yaml"),
        help="特征构建配置文件路径",
    )
    args = parser.parse_args()

    print("=" * 60)
    print("real_raw_1000 Tabular 数据集构建")
    print("=" * 60)
    ...
    features_dir = PROJECT_ROOT / output_paths_cfg.get("features_dir", "data/features/real_raw_1000")
    check_dir = PROJECT_ROOT / output_paths_cfg.get("check_dir", "outputs/data_check/real_raw_1000")
    ...
    # 2. 加载所有表
    # =========================================================================
    print(f"\n[2/10] 加载所有表 ({len(tables_cfg)} 张)...")
    tables = load_all_tables(data_root, tables_cfg)

    # =========================================================================
    # 3. 构建主表 + 字段质量分析 + 低质量字段排除
    # =========================================================================
    print(f"\n[3/10] 构建主表并排除低质量字段...")

    main_df = tables["video_detail"].copy()
    ...
    # 4d. raw_video_status_control — 直接 LEFT JOIN（保留有区分度的字段）
    ...
    if len(sc_df.columns) > 1:  # 至少有 video_id + 至少一个特征
    ...
    comment_df = drop_low_quality_fields(
        ...
        min_non_null_rate=min_non_null_rate,
        drop_constant=drop_constant,
        always_keep=["video_id", "comment_text", "comment_digg_count", "comment_user_id"],
    )
    ...
    id_exceptions = {"video_id", "author_id", "sample_id", "label"}
    ...
    id_exceptions_cat = {"video_id", "author_id", "sample_id", "split"}
    ...
    # none-match 影响
    warnings.append(
        "none-match 样本 212 条 (21.2%) 中，176 条 interaction 字段全为 0/NaN，"
        "36 条有部分互动数据。"
    )
    # play_count 覆盖
    if "play_count" in main_df.columns:
        pc_zero = (main_df["play_count"] == 0).sum()
        warnings.append(f"play_count: {pc_zero}/{len(main_df)} 条为 0 ({pc_zero/len(main_df)*100:.1f}%)")
    # region 覆盖（前向填充前数据）
    if "region" in tables.get("video_detail", pd.DataFrame()).columns:
        vd_region_null = tables["video_detail"]["region"].isna().sum()
        vd_total = len(tables["video_detail"])
        warnings.append(f"region 原始缺失 {vd_region_null}/{vd_total} ({vd_region_null/vd_total*100:.1f}%)，已填充为 __MISSING__")
    # 标签分布
    ...
    # 样本量
    warnings.append(
        "当前样本量 1000 条，train/val/test 三路切分后 test 约 150 条，"
        "泛化评估稳定性有限。"
    )
    # 空表
    ...
    feature_info = {
        ...
        "known_limitations": [
            ...
            "raw_video_tag 当前为空。",
            "raw_chapter 当前为空。",
            "none-match 样本占 21.2%，176 条无互动数据。",
            "评论和相关推荐不是每个视频都触发。",
            "部分字段低覆盖（region 36.5%, item_title 60.7%），已自动过滤最低覆盖字段。",
    ...
    # 9c. tabular_dataset_report.json
    report = {
        "dataset_name": dataset_name,
        "run_id": run_id,
    ...
    report_path = check_dir / "real_raw_1000_tabular_dataset_report.json"
    with open(report_path, "w", encoding="utf-8") as f:
        json.dump(report, f, ensure_ascii=False, indent=2)
    print(f"  tabular_dataset_report.json: {report_path}")

    # 9d. preview CSV
    preview_path = check_dir / "real_raw_1000_tabular_dataset_preview.csv"
