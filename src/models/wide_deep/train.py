...
def main() -> None:
    parser = argparse.ArgumentParser(description="Wide & Deep 训练")
    parser.add_argument(
        "--config",
        type=str,
    ...
    # Deep 数值特征 = numeric_cols + text_stat_cols - 排除项
    ...
    # Deep 类别特征 = categorical_cols - 排除项
    ...
    # Wide 交叉特征 = wide_cross_cols（不经过 excluded 过滤，因为是预计算交叉特征）
    wide_cols = feature_info.get("wide_cross_cols", [])
    ...
    feature_config["deep_hidden_units"] = config.get("deep_hidden_units", [64, 32])
    feature_config["dropout"] = config.get("dropout", 0.3)
    feature_config["wide_embedding_dim"] = config.get("wide_embedding_dim", 1)
    feature_config["warnings"] = [
        "当前数据集仅 79 条样本，所有指标仅用于流程验证",
        "标签为 interaction_score 伪标签，不代表真实业务目标",
    ]
    feature_config["notes"] = [
        "当前 Wide & Deep 基于 sample0427 样本数据训练，仅用于流程级验证。",
