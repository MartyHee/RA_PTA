"""Wide & Deep 训练主程序 — 支持 train/val/test 三路切分"""
...
from models.wide_deep.dataset import (  # noqa: E402
    WideDeepDataProcessor,
    WideDeepDataset,
    get_excluded_cols,
...
logger = get_logger("wide_deep_train")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Wide & Deep 训练与评估")
    parser.add_argument(
        "--config",
        type=str,
    ...
    model_name = config.get("model_name", "wide_deep")
    ...
    if is_three_way:
        ...
        wide_cols = feature_lists["wide_cols"]
        ...
        wide_cols = list(feature_info.get("wide_cross_cols", []))
    ...
    logger.info(
        f"Deep 数值特征数: {len(numeric_cols)}, "
        f"Deep 类别特征数: {len(categorical_cols)}, "
        f"Wide 交叉特征数: {len(wide_cols)}"
    )
    logger.info(f"Deep 数值特征: {numeric_cols}")
    logger.info(f"Deep 类别特征: {categorical_cols}")
    logger.info(f"Wide 交叉特征: {wide_cols}")
    ...
    wide_cols = validate_columns(train_df, wide_cols, "wide")

    # ── 8. 拟合处理器（仅 train） + 转换数据 ────────────────
    processor = WideDeepDataProcessor(
    ...
    train_dataset = WideDeepDataset(
        train_data["numeric"], train_data["categorical"], train_data["wide"],
        train_data["labels"],
    )
    val_dataset = WideDeepDataset(
        val_data["numeric"], val_data["categorical"], val_data["wide"],
        val_data["labels"],
    )
    ...
    if is_three_way and test_df is not None:
        ...
        test_dataset = WideDeepDataset(
            test_data["numeric"], test_data["categorical"], test_data["wide"],
            test_data["labels"],
        )
    ...
    feature_config["notes"] = [
        f"当前 Wide & Deep 基于 {dataset_name} 数据训练",
    ]
    ...
    run_meta = {
        ...
        "feature_profile": {
            ...
            "wide_count": len(wide_cols),
            "text_stat_count": len(text_stat_cols_used),
            "total_features": len(numeric_cols) + len(categorical_cols) + len(wide_cols),
